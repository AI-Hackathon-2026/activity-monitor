...
def client_ip_info(request: Request) -> dict:
    ...
    return {
        "client_ip": chain[0] if chain else (request.client.host if request.client else "unknown"),
        "ip_chain": chain,                # e.g. ["203.0.113.5", "10.0.0.2"] — original client first, each proxy after
        "direct_peer": request.client.host if request.client else None,  # last hop before this app
    }
...
async def post_whoami(whoami_request: WhoamiRequest, whoami_use_case: WhoamiUseCaseDependency, request: Request) -> WhoamiResponse:
